...
{
	int tag, index, offset, w, s, k;

    k = nr_words_per_block;
    s = log2_discrete(nr_sets);
    w = log2_discrete(4*k);
    index = (~(0xffff << s) & (addr >> w));
    tag = addr >> (w+s);
	offset = (~(0xffff << w) & addr);

    for(int j=(index*nr_ways); j<((index*nr_ways)+nr_ways); j++)
    {
		if(cache[j].valid == 0)
		{
			for(int a=0; a < (k * 4); a++)
			{
				...
				cache[j].data[a] = memory[addr + a];
			}
			cache[j].tag = tag;
			cache[j].valid = 1; 
            cache[j].timestamp = cycles;
			return CACHE_MISS;
		}
		else
		{
    ...
    int LRUIndex = 0;
	int min = 10000000;
	for(int a=(index*nr_ways); a<((index*nr_ways)+nr_ways); a++)//캐시 블록 중 timestamp(LRU)가 가장 작은 값의 인덱스 구하기
	{
		if(cache[a].timestamp < min)
		{
			min = cache[a].timestamp;
			LRUIndex = a;	
		}
	}
    if(cache[LRUIndex].dirty == 1)//write-back
    {
		int addr1;
		addr1 = ((cache[LRUIndex].tag << (w+s)) | (index << w) | offset);
		for(int a=0; a < (k * 4); a++)
		{
			if((addr1 % (4*k)) != 0)
		    {
	    		addr1 = addr1 - (addr1 % (4*k));
	    	}
			memory[addr1 + a] = cache[LRUIndex].data[a];
		}
        cache[LRUIndex].dirty = 0; 
		...
		for(int a=0; a < (k * 4); a++)
    	{
    		...
    		cache[LRUIndex].data[a] = memory[addr + a]; 
		}
...
int store_word(unsigned int addr, unsigned int data)
{
	...
	for(int i=0; i<nr_sets; i++)
    {
        for(int j=(index*nr_ways); j<((index*nr_ways)+nr_ways); j++)
        {
			if(cache[j].valid == 0)//write-allocate
			{
				for(int a=0; a < (k * 4); a++)
				{
					if((addr % (4*k)) != 0)
				...
				for(int a=(addr&0x0f); a < ((addr&0x0f)+4); a++)
				{
					cache[j].data[a] = ((data >> (8*(3-a))) & 0x000000ff);
				}
				cache[j].tag = tag;
				cache[j].valid = 1;
                cache[j].dirty = 1;
        ...
        if(cache[LRUIndex].dirty == 1)//write-back
        {
			...
			for(int a=0; a < (k * 4); a++)
			{
				...
				memory[addr1 + a] = cache[LRUIndex].data[a];
				cache[LRUIndex].data[a] = NULL;
			}
			for(int a=(addr&0x0f); a < ((addr&0x0f)+4); a++)
	    	{
	    		cache[LRUIndex].data[a] = ((data >> (8*(3-a))) & 0x000000ff);
    		}
    	    cache[LRUIndex].tag = tag;
            cache[LRUIndex].timestamp = cycles;
	        return CACHE_MISS; 
